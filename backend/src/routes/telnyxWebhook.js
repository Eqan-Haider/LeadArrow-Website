...
try {
  ...
  console.warn('[telnyxWebhook] Socket.io not available');
}
  ...
  try {
    ...
    if (data.event_type === 'call.answered') {
      ...
      if (digit === '1') {
        ...
        if (io) {
          io.emit('CALL_STATUS', { attemptId: attempt.id, status: 'ACCEPTED' });
        }
        ...
        if (io) {
          io.emit('CALL_STATUS', { attemptId: attempt.id, status: 'DECLINED' });
        }
